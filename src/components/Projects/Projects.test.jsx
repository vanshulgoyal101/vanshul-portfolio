...
describe('Projects', () => {
  ...
  it('shows the canonical AdBrain icon without duplicating its accessible name', () => {
    ...
    expect(screen.getByRole('img', { name: 'AdBrain', exact: true })).toHaveAttribute('src', '/images/projects/adbrain.webp?v=edge-to-edge-20260916');
  });
  ...
  it('provides a case study for each featured project and keeps the directory secondary', () => {
    ...
    expect(screen.getByRole('region', { name: "More things I've built" })).not.toHaveAttribute('open');
  });
  ...
  it('keeps secondary projects collapsed until expanded and allows closing again', () => {
    render(<Projects />);
